...
- **月视图预览返回不再整屏刷新（小程序 v20260820.8）**：`wx.previewImage`
  ...
  后返回也不会整屏闪白。
- **上传完整性 + fsync + 删除收敛（dav-bridge 10 / 小程序 v20260820.9）**：
  - 上传带 `md5`/`size`（`wx.getFileInfo`），CGI 边写边算，不一致 400 拒绝
    发布（损坏文件不进列表）；发布前 fsync 文件、rename 后 fsync 目录；
  - 删除走桥端 `op=delete`（DELETE）：一次删主文件 + 全部派生文件，客户端
    并发 3 路，原来逐文件 4 次串行 → 每文件 1 次；
  - 上传时同月目录只在会话内 mkcol 一次（原来每个文件打一次）；
  - 新增依赖 `libopenssl`（MD5）。
  **板子待部署**：重编 dav-bridge 10（`make ... V=s` + `make package/index`）
  + 上传小程序体验版 v20260820.9。
