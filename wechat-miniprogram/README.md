...
登录页“记住密码”勾选（默认开）：勾选时把服务器地址/账号/密码存到独立的
`davSavedCred` storage；下次打开登录页自动回填。取消勾选登录会清除已保存
的凭据。该存储独立于登录会话（`davSession`），退出登录/会话失效不影响回填。

## IPv4 / IPv6 双栈（2026-08-20）

- 域名同时有 A（家里路由器公网 IPv4）与 AAAA（板子 WAN IPv6）记录，
  由板子 ddns-go 维护（`-f 60` 刷新）。
- 无 IPv6 的网络（或切网瞬间 IPv6 未就绪）首次请求可能直接
  “网络不可达”：`utils/wxreq.js` 已对网络层失败自动重试
  （请求 3 次、间隔递增；上传仅在“连接未建立”时重试 2 次，超时不重试
  避免重复文件），登录页会区分 401 / 不可达 / 超时提示。
